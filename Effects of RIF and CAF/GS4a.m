...
% Studying the effects of administration of Rifampicin
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% cell # definition
sleep_hours = 8; % Average number of hours the person sleeps
N = zeros(t,1);
M = zeros(t,1);
EC = zeros(t,1);
...
C10 = zeros(t,1);
CRif = zeros(t,1);  % conc. of Rifampicin in blood serum


LRPEC = zeros(t,1); % LRP density on brain ECs
LRPEC_basal = zeros(t,1);
...
C10(1,1) = 0;
CRif(1,1) = 0;              % Initial conc of Rifampicin 

LRPEC(1,1) = 9.3e11;       % # of LRP1 receptors per endothelial cell
LRPEC_basal(1,1) = 9.3e11;       % # of LRP1 receptors per endothelial cell
...
k_LRPac = 1;        % 1 hr^-1 is taken as rate constant 
k_LRPdeg = 1/24;    % hr^-1 from cal et al
kpgp_increment = 2.7; % Increase in Flow rates 
...
for n = 1:m
    ...
    if CRif(n,1) >= 0                    
        CRif(n+1,1) = CRif(n,1) - (log(2)/2.5)*CRif(n,1);
    else 
        CRif(n+1,1) =0;
    end
    ...
    LRPEC_basal(n+1,1) = (1-k10)*LRPEC_basal(n,1);
    if CRif(n,1) == 0
        LRPEC(n+1,1) = (1-k10)*LRPEC(n,1); %*ratio;   %(EC(n,1)./(EC(1,1)));  % change in LRP1 # wrt aging
    elseif CRif(n,1) > 1
        LRPEC(n+1,1) = (1-k10)*LRPEC(n,1) + k_LRPac*(2*LRPEC_basal(n,1) - LRPEC(n,1)); % Upper limit from Qosa et al.
    elseif CRif(n,1)>0 && CRif(n,1)<1
        if LRPEC(n,1) > LRPEC_basal(n,1)
            LRPEC(n+1, 1) = (1-k10)*LRPEC(n,1) - k_LRPdeg*(LRPEC(n,1)-LRPEC_basal(n,1));
        else
            LRPEC(n+1,1) = LRPEC_basal(n,1);
        end
    end
...
figure (3)
plot(x, LRPEC,x,LRPEC_basal, 'Linewidth', 2)
