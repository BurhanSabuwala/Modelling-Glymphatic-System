...
CAPP = zeros(t,1);  % conc. of Amyloid Precursor Protein
C99 = zeros(t,1);   % conc. of C99 
...
C10 = zeros(t,1);
CAl = zeros(t,1);   % Conc. of Al ions in Blood/ BBB
...
CAPP(1,1) = 3e3;         % APP in Neurons
C99(1,1) = 3e3;          % C99 in neurons
...
C10(1,1) = 0;
CAl(1,1) = 0;              % initial Concentration of Al ions
...
kAPP = 1888;      % Production rate of APP, APP per neuron per hr in WT
kAPP_factor = 10;  % kn_factor = kn_wake/kn_sleep
kC99 = 0.666;      % Conversion of APP to C99 per hour (First order rate constant)
vC99 = 0.333;      % Degradation rate of C99 per hour
kAb40 = 0.240;     % Conversion of C99 to Ab40 per hour (First order reaction)
kAb42 = 0.045;     % Conversion of C99 to Ab42 per hour (First order reaction)
...
%k6 = 0.12/24;      % per hr, fraction of Ab removed by LRP1 from PVS when ApoE4 allele present (Bachmeir 2012)

k7 = 0.004/24;     % deposition rate of Ab40 per hr in brain vessel
k9 = 0.001/24;     % deposition rate of Ab42 per hr in brain i
...
del_Al_inc = 0.2; % Arbitary increase in rate of APP prod due to presence Al3+ ions
t_half_Al = 100*24; % Half life of Al in hours
...
for n = 1:m
    ...
    % Al dynamics
    if CAl(n,1)>0
        CAl(n+1,1) = CAl(n,1) - (log(2)/t_half_Al)*CAl(n,1);
    else
        CAl(n+1,1) = 0;
    end
    if rand()<0.0088 && n <365*24*40
        CAl(n+1,1) = CAl(n,1) + 1;           % Unit in ug/ml (Normal is 1 ug/ml)
    end
    % APP levels in the neuron
    if CAPP(n,1)>=0 && CAPP(n,1) >= 5*CAPP(1,1)
        if rem(n,24)<16
            kAPP_wake = 24*kAPP_factor*kAPP/((24-sleep_hours)*kAPP_factor + sleep_hours);
            if CAl(n) > 10
                CAPP(n+1, 1) = CAPP(n,1) + k4*kAPP_wake + del_Al_inc*kAPP_wake - kC99*CAPP(n,1);
            elseif CAl(n)<= 10 && CAl(n) >2
                    CAPP(n+1, 1) = CAPP(n,1) + k4*kAPP_wake + del_Al_inc*kAPP_wake*(CAl(n)/10) - kC99*CAPP(n,1);
            else
                    CAPP(n+1, 1) = CAPP(n,1) + k4*kAPP_wake - kC99*CAPP(n,1);
           
            end
        else 
            kAPP_sleep = 24*kAPP/((24 - sleep_hours)*kAPP_factor + sleep_hours);
            if CAl(n) > 10
                CAPP(n+1, 1) = CAPP(n,1) + k4*kAPP_sleep + del_Al_inc*kAPP_sleep - kC99*CAPP(n,1);
            elseif CAl(n)<= 10 && CAl(n) >2
                    CAPP(n+1, 1) = CAPP(n,1) + k4*kAPP_sleep + del_Al_inc*kAPP_sleep*(CAl(n)/10) - kC99*CAPP(n,1);
            else
                    CAPP(n+1, 1) = CAPP(n,1) + k4*kAPP_sleep - kC99*CAPP(n,1);
            end
    ...
    elseif CAPP(n,1) >= 0
        if rem(n,24)<16
            kAPP_wake = 24*kAPP_factor*kAPP/((24-sleep_hours)*kAPP_factor + sleep_hours);
            if CAl(n) > 10
                CAPP(n+1, 1) = CAPP(n,1) + kAPP_wake + del_Al_inc*kAPP_wake - kC99*CAPP(n,1);
            elseif CAl(n)<= 10 && CAl(n) >2
                CAPP(n+1, 1) = CAPP(n,1) + kAPP_wake + del_Al_inc*kAPP_wake*(CAl(n)/10) - kC99*CAPP(n,1);
            else
                CAPP(n+1, 1) = CAPP(n,1) + kAPP_wake - kC99*CAPP(n,1);
           
            end
        else 
            kAPP_sleep = 24*kAPP/((24 - sleep_hours)*kAPP_factor + sleep_hours);
            if CAl(n) > 10
                CAPP(n+1, 1) = CAPP(n,1) + kAPP_sleep + del_Al_inc*kAPP_sleep - kC99*CAPP(n,1);
            elseif CAl(n)<= 10 && CAl(n) >2
                    CAPP(n+1, 1) = CAPP(n,1) + kAPP_sleep + del_Al_inc*kAPP_sleep*(CAl(n)/10) - kC99*CAPP(n,1);
            else
                    CAPP(n+1, 1) = CAPP(n,1) + kAPP_sleep - kC99*CAPP(n,1);
            end
    ...
    % C99 levels
    if C99(n,1)>=0
        C99(n+1,1) = C99(n,1) + kC99*CAPP(n,1) - (vC99 + 0.062 + kAb40 + kAb42)*C99(n,1);
    else
        C99(n+1,1) = 0;
    end
    ...
    %        C1(n+1,1) = C1(n,1) + kAb40*C99(n,1)*N(n,1) - k5*M(n,1) - Fi40(n,1) - k7a2*C1(n,1);
    ...
    %        C4(n+1,1) = C4(n,1) + kAb42*C99(n,1)*1.01*N(n,1) - k11*M(n,1) - Fi42(n,1);
    %    else
...
xlabel('Hours'), ylabel('Ab40/Ab42');
hold on

figure(6)
plot(x,CAl,'r', x,CAPP, 'g', 'Linewidth',1)
hold on
% figure(4)
% plot(x,C1, '+')
% hold on
