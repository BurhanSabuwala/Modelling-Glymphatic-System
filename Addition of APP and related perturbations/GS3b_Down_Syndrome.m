...
%end
stiffness = 1;
velocity = 0.14*(HR/60)*(1/stiffness)/24;      % mm per hour, velocity of bulk ISF flow (from Abbott 2004)
...
kAPP = 1888;      % Production rate of APP, APP per neuron per hr
kAPP = 1.5 * 1888;      % Production rate of APP, APP per neuron per hr in case of Down Syndrome
...
for n = 1:m
    ...
    %   if rem(n,24)<16
    %        C1(n+1,1) = C1(n,1) + k4*kAb40*C99(n,1)*N(n,1) - k5*M(n,1) - Fi40(n,1) - k7a2*C1(n,1);
    ...
    %        C4(n+1,1) = C4(n,1) + k4*kAb42*C99(n,1)*1.01*N(n,1) - k11*M(n,1) - Fi42(n,1);
    %    else
