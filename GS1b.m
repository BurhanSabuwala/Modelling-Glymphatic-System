...
for n = 1:m
    if rem(n,24)<16
        velocity_wake = velocity*3*v_factor/(2*v_factor +1);
        ...
        velocity_sleep = velocity*3/(2*v_factor+1);
    ...
    if C1(n,1) >=0 && C1(n,1)>=5*C1(1,1)
        if rem(n,24)<16
            k3_wake = 3*k3_factor*k3/(2*k3_factor + 1); % generation rate of Ab40 per neuron per hour during wake
            C1(n+1,1) = C1(n,1) + k4*k3_wake*N(n,1) - k5*M(n,1) - Fi40(n,1) - k7a2*C1(n,1);
            C9(n+1,1) = C9(n,1) + k7a2*C1(n,1);     %Ab40 deposition in brain
        else
            k3_sleep = 3*k3/(2*k3_factor + 1);       % generation rate of Ab40 per neuron per hour during sleep
            C1(n+1,1) = C1(n,1) + k4*k3_sleep*N(n,1) - k5*M(n,1) - Fi40(n,1) - k7a2*C1(n,1);
            C9(n+1,1) = C9(n,1) + k7a2*C1(n,1);     %Ab40 deposition in brain
        end 
    elseif C1(n,1) >=0
        if rem(n,24)<16
            k3_wake = 3*k3_factor*k3/(2*k3_factor+1);       % generation rate of Ab40 per cell per hour
            C1(n+1,1) = C1(n,1) + k3_wake*N(n,1) - k5*M(n,1) - Fi40(n,1) - k7a2*C1(n,1);
            C9(n+1,1) = C9(n,1) + k7a2*C1(n,1);     %Ab40 deposition in brain
        else
            k3_sleep = 3*k3/(2*k3_factor+1);       % generation rate of Ab40 per cell per hour
